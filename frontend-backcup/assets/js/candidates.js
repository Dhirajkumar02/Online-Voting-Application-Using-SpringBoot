...
function loadCandidates() {
	fetch("http://localhost:8082/api/candidate")
		.then((res) => res.json())
		.then((data) => {
...
document
	...
	?.addEventListener("submit", function (event) {
		...
		fetch("http://localhost:8082/api/candidate/add", {
			method: "POST",
			headers: {
...
function searchCandidate() {
	...
	fetch(`http://localhost:8082/api/candidate/${candidateId}`)
		.then((res) => res.json())
		.then((candidate) => {
			Swal.fire({
...
// 	fetch(`http://localhost:8082/api/candidate/${id}`, {
// 		method: "PUT",
...
function updateCandidate(candidateId, newName, newParty) {
	fetch(`http://localhost:8082/api/candidate/update/${candidateId}`, {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ name: newName, party: newParty }),
	})
...
function deleteCandidate(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			fetch(`http://localhost:8082/api/candidate/delete/${id}`, {
				method: "DELETE",
			})
				.then(() => {
